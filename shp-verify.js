    ...
    const SHP = {
        ...
        showIndicator(valid) {
            ...
            if (valid) {
                ...
                indicator.innerHTML = '🔒 Verified by SHP';
                indicator.title = 'Content signature is valid. Strict mode enabled.';
            } else {
                indicator.style.background = '#ef4444';
                indicator.style.color = '#fff';
                indicator.innerHTML = '⚠️ Invalid Signature';
        ...
        async validate() {
            ...
            if (isValid) {
                console.log('[SHP] ✓ Signature valid. Strict mode enabled.');
                document.documentElement.setAttribute('data-shp-mode', 'strict');
            } else {
                console.log('[SHP] ✗ Signature invalid. Legacy mode active.');
